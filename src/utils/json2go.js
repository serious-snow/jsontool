...
import { getComment, toCamel } from "@/utils/util";

const TYPEMap = {};

TYPEMap[TYPE.Struct] = "struct";
TYPEMap[TYPE.Object] = "any";
...
export const json2go = (object, option = {}) => {
  return getJson(getTarget("", object, -1, getComment("", object)), option);
};
...
const getFieldName = (ob = {}, option = {}) => {
  ...
  switch (ob.type) {
    case TYPE.Struct:
      ob.key
        ? result.push(`struct {`)
        : result.push(`type AutoGenerated struct {`);
      ...
      return result.join("\n");
    case TYPE.Slice:
      return `${ob.key ? "[]" : "type AutoGenerated []"}${
        ob.item
          ? getFieldName(ob.item, option)
          : getFieldName(new object(), option)
      }`;
    case TYPE.Object:
      if (!option.useAny) {
        return "interface{}";
...
const getJson = (ob, option) => {
  ...
  result.push(
    `${prefix}${toCamel(ob.key)} ${fieldName} ${jsonTag} ${afterComment}`
  );
